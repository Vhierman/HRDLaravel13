    ...
    <div class="card">
        <div class="card-body">
            <div class="row row-cols-auto g-3">
                <div class="col">
                    <form action="{{ route('training_eksternal.excell_view_penempatan') }}" method="POST" class="d-inline">
                        @csrf
                        <input type="hidden" name="divisions_id" class="form-control" value="{{ $divisions_id }}">
                        <button type="submit" class="btn btn-success px-5 raised">
                            Download Rekap Training eksternal {{ $penempatan }}
                        </button>
                    </form>
                </div>
            </div>
