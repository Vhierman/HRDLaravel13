    ...
    <div class="card">
        <div class="card-body">
            <div class="row row-cols-auto g-3">
                <div class="col">
                    <form action="{{ route('training_eksternal.hapus_tanggal') }}" method="POST" class="d-inline">
                        @csrf
                        <input type="hidden" name="tanggal_awal_training_eksternal" class="form-control"
                            value="{{ $tanggal_awal_training_eksternal }}">

                        <button type="submit" class="btn btn-danger px-5 raised">
                            Hapus Data Training eksternal Tanggal
                            {{ \Carbon\Carbon::parse($tanggal_awal_training_eksternal)->isoformat('DD-MM-Y') }}
                        </button>
                    </form>
                </div>
            </div>
                ...
                <table id="example2" class="table table-striped table-bordered">
                    <thead>
                        ...
                        <tr class="search-row">
                            ...
                            <th><input type="text" class="form-control form-control-sm"
                                    placeholder="Cari Lokasi Training..." />
                            </th>
                    ...
                    <tbody>
                        ...
                        @foreach ($item_training_eksternals as $item_training_eksternal)
                            <tr>
                                ...
                                <td>{{ \Carbon\Carbon::parse($item_training_eksternal->tanggal_training_eksternal)->isoformat('DD-MM-Y') }}
                                </td>
...
{{-- Datatables --}}
@section('css')
    <link href="{{ asset('template_admin/assets/plugins/datatable/css/dataTables.bootstrap5.min.css') }}"
        rel="stylesheet" />
    <style>
        .search-row input {
            width: 100%;
