...
@section('title', 'Edit Data Overtime');
@section('content')
            ...
            <form action="{{ route('overtime.update', $item_overtime->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="row g-3 mt-2">
                    <div class="col-12 col-lg-6">
                        ...
                        <select id="input39" class="form-select" name="jenis_lembur">
                            <option value="">Pilih Jenis Lembur</option>
                            <option value="Biasa" @if ($item_overtime->jenis_lembur == 'Biasa') {{ 'selected="selected"' }} @endif>
                                Biasa</option>
                            <option value="Libur" @if ($item_overtime->jenis_lembur == 'Libur') {{ 'selected="selected"' }} @endif>
                                Libur</option>
                        </select>
                    </div>
                    <div class="col-12 col-lg-6">
                        <label class="form-label fs-6">Uang Makan Lembur</label>
                        <select id="input39" class="form-select" name="uang_makan_lembur">
                            <option value="">Pilih Uang Makan Lembur</option>
                            <option value="17000" @if ($item_overtime->uang_makan_lembur == '17000') {{ 'selected="selected"' }} @endif>
                                Dapat Uang Makan</option>
                            <option value="0" @if ($item_overtime->uang_makan_lembur == '0') {{ 'selected="selected"' }} @endif>
                                Tidak Dapat Uang Makan</option>
                        </select>
                    </div>
                </div>
                <div class="row g-3 mt-2">
