    ...
    <div class="card">
        <div class="card-body">
            <div class="row row-cols-auto g-3">
                <div class="col">
                    <form action="{{ route('pemeriksaan_karyawan.export_excell') }}" method="POST" class="d-inline">
                        @csrf
                        ...
                        <button type="submit" class="btn btn-success px-5 raised">
                            Download Rekap Pemeriksaan Karyawan Periode
                ...
                <table id="example2" class="table table-striped table-bordered align-middle" style="width:100%">
                    <thead>
                        ...
                        <tr class="search-row">
                            ...
                            <th><input type="text" class="form-control form-control-sm"
                                    placeholder="Cari Status Kelayakan..." />
                            </th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
                        @php
                            $no = 1;
                        @endphp
                        @foreach ($itemPemeriksaans as $itemPemeriksaan)
                            <tr>
                                <td>{{ $no++ }}</td>
                                <td>{{ $itemPemeriksaan->employees->nama_karyawan }}</td>
                                <td>{{ $itemPemeriksaan->nik_karyawan }}</td>
                                <td>{{ $itemPemeriksaan->employees->divisions->penempatan }}</td>
                                <td>{{ \Carbon\Carbon::parse($itemPemeriksaan->tanggal_pemeriksaan)->isoformat('DD-MM-Y') }}
                                </td>
                                <td>{{ $itemPemeriksaan->faskes->nama_faskes }}</td>
                                <td>{{ $itemPemeriksaan->jenis_pemeriksaan }}</td>
                                <td>{{ $itemPemeriksaan->status_kelayakan }}</td>
                                <td>
                                    <div class="action-container">
                                        <button type="button"
                                            onclick="window.location.href='{{ route('pemeriksaan_karyawan.edit', $itemPemeriksaan->id) }}'"
                                            class="btn btn-sm btn-outline-success btn-action" title="Edit Data">
                                            <i class="material-icons-outlined">edit</i>
                                        </button>
                                        <form action="{{ route('pemeriksaan_karyawan.destroy', $itemPemeriksaan->id) }}"
                                            method="POST" class="d-inline delete-form" style="display: inline; margin: 0;">
                                            @csrf
                                            @method('delete')
                                            <button type="button"
