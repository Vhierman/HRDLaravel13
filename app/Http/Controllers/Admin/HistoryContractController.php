...
use App\Http\Requests\Admin\HistoryContractRequest;
...
class HistoryContractController extends Controller
{
    /**
     ...
     */
    public function store(HistoryContractRequest $request)
    {
        //
        if (auth()->user()->roles != 'admin' && auth()->user()->roles != 'hrd') {
            abort(403);
    ...
    public function destroy(string $id)
    {
        ...
        $history = HistoryContracts::findOrFail($id);
        $employeeId = $history->employees_id; 
    ...
    public function pkwt($id)
    {
        ...
        $item_history_kontrak   = HistoryContracts::findOrFail($id);
        $nik_karyawan           = $item_history_kontrak->nik_karyawan;
        $status_kontrak_kerja   = $item_history_kontrak->status_kontrak_kerja;

        $karyawan               = Employees::with(['divisions','positions'])->where('nik_karyawan', $nik_karyawan)->first();
        
        $tanggal_salary         = substr($item_history_kontrak->tanggal_awal_kontrak, 8, 3);
        $bulan_salary           = substr($item_history_kontrak->tanggal_awal_kontrak, 5, 2);
        $tahun_salary           = substr($item_history_kontrak->tanggal_awal_kontrak, 0,4);
        ...
        if ($status_kontrak_kerja == "PKWTT")
        {
            ...
            $this->fpdf->Cell(70);
            $this->fpdf->Cell(70, 7, 'No : ' . $nomor . '/HRD/PK/' . $romawi . '/' . $tahun . '', 0, 0, 'R');

            $this->fpdf->SetFont('Arial', 'B', '12');
            $this->fpdf->Ln(5);
            $this->fpdf->Cell(10);
            $this->fpdf->Cell(40, 7, $kepada . '' . $karyawan->nama_karyawan . '', 0, 0, 'L');
            ...
            $text = "Terhitung sejak tanggal " .
                \Carbon\Carbon::parse($item_history_kontrak->tanggal_awal_kontrak)->isoFormat('D MMMM Y') .
                ", Management PT Prima Komponen Indonesia mengangkat saudara " .
                $karyawan->nama_karyawan .
                " sebagai Karyawan Tetap setelah menjalani masa kontrak kerja sejak tanggal " .
                \Carbon\Carbon::parse($karyawan->tanggal_mulai_kerja)->isoFormat('D MMMM Y') .
                ".";

            $this->fpdf->SetX(12);
            $this->fpdf->MultiCell(180, 7, $text, 0, 'J');
            ...
            $text1 = "Hak saudara sebagai karyawan tetap mulai berlaku sejak tanggal pengangkatan karyawan tetap ini, yaitu (Pesangon, Penghargaan Masa Kerja, dan lain-lain apabila terjadi pengakhiran masa kerja). Namun masa kerja saudara berlaku untuk perhitungan masa cuti yang belum diambil sejak kontrak di tandatangani.";
            $this->fpdf->SetX($indentTeks); 
            $this->fpdf->MultiCell($lebarTeks, 7, $text1, 0, 'J');
            // Nomor1
            ...
            $this->fpdf->SetX($indentTeks);
            $this->fpdf->Cell(50, 7, 'Jabatan / Nama Bagian : ', 0, 0, 'L');
            // Input Dinamis (Bold)
            $this->fpdf->SetFont('Arial', 'B', 12);
            $this->fpdf->Cell(0, 7, $karyawan->positions->jabatan.'/'.$karyawan->divisions->penempatan, 0, 1, 'L');
            $this->fpdf->SetFont('Arial', '', 12); // Kembalikan ke normal
            // Nomor2

            // Nomor3
            $this->fpdf->Ln(1); 
            $this->fpdf->SetX($indentNomor);
            $this->fpdf->Cell(10, 7, '3.', 0, 0, 'L');
            $this->fpdf->SetX($indentTeks);
            $this->fpdf->MultiCell($lebarTeks, 7, 'Sistem pengupahan adalah azas No Work No Pay.', 0, 'J');
            // Nomor3
            ...
            $lebarTeks = 165;
            $this->fpdf->SetX($indentNomor);
            $this->fpdf->Cell(10, 7, '4.', 0, 0, 'L');
            $text1 = "Wajib mengikuti/mematuhi tata tertib yang berlaku pada pihak perusahaan serta undang-undang ketenagakerjaan yang berlaku.";
            $this->fpdf->SetX($indentTeks); 
            $this->fpdf->MultiCell($lebarTeks, 7, $text1, 0, 'J');
            // Nomor4
            ...
            $this->fpdf->Ln(1); 
            $this->fpdf->SetX($indentNomor);
            $this->fpdf->Cell(10, 7, '5.', 0, 0, 'L');
            $this->fpdf->SetX($indentTeks);
            $this->fpdf->MultiCell($lebarTeks, 7, 'Berhak mendapat cuti tahunan 12 hari kerja dalam satu tahun masa kerja.', 0, 'J');
            //Nomor5

            //Nomor6
            $this->fpdf->Ln(1); 
            $this->fpdf->SetX($indentNomor);
            $this->fpdf->Cell(10, 7, '6.', 0, 0, 'L');
            $this->fpdf->SetX($indentTeks);
            $this->fpdf->MultiCell($lebarTeks, 7, 'Berhak mendapat asuransi tenaga kerja sesuai dengan ketentuan undang-undang yang berlaku.', 0, 'J');
            //Nomor6

            $this->fpdf->Ln(5); 
            $text = "Hal-hal yang belum jelas pada surat pengangkatan ini akan dijelaskan kemudian secara lisan atau tertulis, Demikianlah surat pengangkatan ini dibuat untuk dapat dimaklumi.";
            $this->fpdf->SetX(12);
            $this->fpdf->MultiCell(180, 7, $text, 0, 'J');
            ...
            $this->fpdf->SetFont('Arial', 'B', '12');
            $this->fpdf->Cell(50);
            $this->fpdf->Cell(100, 7, 'Dikeluarkan di Tangerang Selatan, Tanggal, ' . \Carbon\Carbon::parse($item_history_kontrak->tanggal_awal_kontrak)->isoformat('D MMMM Y'), 0, 0, 'C');
        ...
        elseif ($status_kontrak_kerja == "PKWT")
        {
            $this->fpdf = new FPDF('P', 'mm', 'A4');
            $this->fpdf->AddPage();
            ...
            $this->fpdf->Cell(30, 5, 'Alamat', 0, 0, 'L');
            $this->fpdf->SetFont('Arial', 'B', '9');
            $this->fpdf->Cell(140, 5, ': ' . $karyawan->alamat . ', RT.' . $karyawan->rt . ' / ' . $karyawan->rw . ', Kelurahan ' . $karyawan->kelurahan . '', 0, 0, 'L');
            $this->fpdf->Ln(5);
            $this->fpdf->Cell(50);
            $this->fpdf->Cell(140, 5, '  Kecamatan ' . $karyawan->kecamatan . ', Kota ' . $karyawan->kota . ', Provinsi ' . $karyawan->provinsi . ',' . $karyawan->kode_pos . '', 0, 0, 'L');
            $this->fpdf->SetFont('Arial', '', '9');
            $this->fpdf->Ln(7);
            
            $this->fpdf->SetX($indentNomor);
            $text1 = "Dalam hal ini bertindak untuk dan atas nama dirinya sendiri dan selanjutnya disebut PIHAK KEDUA. Pada hari ".\Carbon\Carbon::parse($item_history_kontrak->tanggal_awal_kontrak)->isoformat('dddd, D MMMM Y')." bertempat di gedung PT Prima Komponen Indonesia, kedua belah pihak dengan ini sepakat untuk mengadakan perjanjian / ikatan kerja dalam jangka waktu tertentu, yaitu melalui kontrak kerja yang hubungan kerjanya berpegang pada syarat - syarat dan ketentuan sebagai berikut : ";
            $this->fpdf->SetX($indentTeks); 
            $this->fpdf->MultiCell($lebarTeks, 5, $text1, 0, 'J');
            ...
            $this->fpdf->SetFont('Arial', 'B', '9');
            $this->fpdf->Cell(20, 5,$item_history_kontrak->masa_kontrak, 0, 0, 'L');
            $this->fpdf->Ln(5);
            $this->fpdf->Cell(20);
            $this->fpdf->SetFont('Arial', '', '9');
            $this->fpdf->Cell(85, 5,'terhitung sejak perjanjian kerja ini ditandatangani yaitu dari ', 0, 0, 'L');
            $this->fpdf->SetFont('Arial', 'B', '9');
            $this->fpdf->Cell(28, 5, \Carbon\Carbon::parse($item_history_kontrak->tanggal_awal_kontrak)->isoformat('D MMMM Y'), 0, 0, 'C');
            $this->fpdf->SetFont('Arial', '', '9');
            $this->fpdf->Cell(25, 5, ' sampai dengan ', 0, 0, 'C');
            $this->fpdf->SetFont('Arial', 'B', '9');
            $this->fpdf->Cell(28, 5,\Carbon\Carbon::parse($item_history_kontrak->tanggal_akhir_kontrak)->isoformat('D MMMM Y'), 0, 0, 'C');
            //Pasal 2
            ...
            $this->fpdf->Cell(60);
            $this->fpdf->SetFont('Arial', 'B', '9');
            $this->fpdf->Cell(70, 5, 'Tangerang Selatan, ' . \Carbon\Carbon::parse($item_history_kontrak->tanggal_awal_kontrak)->isoformat('D MMMM Y'), 0, 0, 'C');
            $this->fpdf->Ln(10);
            $this->fpdf->Cell(20);
            $this->fpdf->SetFont('Arial', '', '9');
        ...
        elseif ($status_kontrak_kerja == "Harian")
        {
            $this->fpdf = new FPDF('P', 'mm', 'A4');
            ...
            $this->fpdf->Cell(5, 7, ' : ', 0, 0, 'C');
            $this->fpdf->Cell(115, 7, ' '.$karyawan->nik_karyawan, 0, 0, 'L');
            ...
            $this->fpdf->Cell(50, 7, 'Nama', 0, 0, 'L');
            $this->fpdf->Cell(5, 7, ' : ', 0, 0, 'C');
            $this->fpdf->Cell(115, 7, ' '.$karyawan->nama_karyawan, 0, 0, 'L');
            ...
            $this->fpdf->Cell(5, 7, ' : ', 0, 0, 'C');
            $this->fpdf->Cell(115, 7, ' '.$karyawan->tempat_lahir.', '.\Carbon\Carbon::parse($karyawan->tanggal_lahir)->isoformat('D MMMM Y'), 0, 0, 'L');
            ...
            $this->fpdf->Cell(5, 7, ' : ', 0, 0, 'C');
            $this->fpdf->Cell(115, 7, ' '.$karyawan->pendidikan_terakhir, 0, 0, 'L');
            ...
            $this->fpdf->Cell(5, 7, ' : ', 0, 0, 'C');
            $this->fpdf->Cell(115, 7, ' '.$karyawan->jenis_kelamin, 0, 0, 'L');
            ...
            $this->fpdf->Cell(5, 7, ' : ', 0, 0, 'C');
            $this->fpdf->Cell(115, 7, ' '.$karyawan->agama, 0, 0, 'L');
            ...
            $this->fpdf->Cell(5, 7, ' : ', 0, 0, 'C');
            $this->fpdf->Cell(115, 7, ' '.$karyawan->alamat.', '.$karyawan->rt.'/'.$karyawan->rw.', Kelurahan.'.$karyawan->kelurahan, 0, 0, 'L');
    
            $this->fpdf->Ln();
            $this->fpdf->Cell(75);
            $this->fpdf->Cell(115, 7, ' Kecamatan.'.$karyawan->kecamatan.', Kota.'.$karyawan->kota.', '.$karyawan->provinsi, 0, 0, 'L');
            ...
            $this->fpdf->Cell(60, 5, ''.$karyawan->positions->jabatan.' / '.$karyawan->divisions->penempatan, 0, 0, 'L');
            ...
            $this->fpdf->SetFont('Arial', 'BU', '11');
            $this->fpdf->Cell(30, 6, ' '.\Carbon\Carbon::parse($item_history_kontrak->tanggal_akhir_kontrak)->isoformat('D MMMM Y').'.', 0, 0, 'L');
            ...
            $this->fpdf->Ln();
            $this->fpdf->Cell(20);
            $this->fpdf->Cell(77, 5, 'Sebagai imbalan kerja lembur sesuai ayat 1,', 0, 0, 'L');
            ...
            $this->fpdf->SetFont('Arial', 'B', '11');
            $this->fpdf->Cell(23, 5, 'Rp.'.number_format($salary->upah_lembur_perjam), 0, 0, 'L');
            $this->fpdf->SetFont('Arial', '', '11');
            $this->fpdf->SetFont('Arial', '', '11');
            ...
            $this->fpdf->SetFont('Arial', '', '11');
            $this->fpdf->Cell(20);
            $this->fpdf->Cell(170, 5, 'Tangerang Selatan, '.\Carbon\Carbon::parse($item_history_kontrak->tanggal_awal_kontrak)->isoformat('D MMMM Y'), 0, 0, 'C');
        ...
        elseif ($status_kontrak_kerja == "Outsourcing")
        {
             return response("
                    <script>
                        window.close();
