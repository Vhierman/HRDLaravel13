    ...
    <div class="card">
        <div class="card-body">
            <div class="row row-cols-auto g-3">
                <div class="col">
                    <form action="{{ route('attendance.export_excell_non_absensi') }}" method="POST" class="d-inline">
                        @csrf
                ...
                <table id="example2" class="table table-striped table-bordered">
                    ...
                    <tbody>
                        ...
                        @foreach ($item_employees as $item_employee)
                            <tr>
                                <td>{{ $no++ }}</td>
                                <td>{{ $item_employee->nama_karyawan }}</td>
                                <td>{{ $item_employee->nik_karyawan }}</td>
                                <td>{{ $item_employee->golongans->golongan }}</td>
                                <td>{{ $item_employee->positions->jabatan }}</td>
                                <td>{{ $item_employee->divisions->penempatan }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
